...
@onready var txt_label = $label_margin/txt_label
...
func display_text(text_to_display: String):
	...
	txt_label.text = text_to_display
	...
	if size.x > MAX_WIDTH:
		txt_label.autowrap_mode = TextServer.AUTOWRAP_WORD
		await resized
		await resized
...
func display_letter():
	txt_label.text += text[letter_index]
	letter_index += 1
